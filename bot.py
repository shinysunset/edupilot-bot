...
BOT_VERSION = "2026-06-13-neutral-guidance-v1"
...
class CognitiveBoundaryManager:
    ...
    def detect_task_domain(self, task_text: str, user_message: str = "") -> str:
        ...
        if self._contains_any(combined, ["физик", "тело", "м/с", "м/с^2", "ньютон", "джоул", "ускорен", "сила", "масса", "давлен", "энерг", "мощност", "ток", "напряжен"]):
            return "physics"
        ...
        if self._contains_any(combined, ["физик", "сила", "масса", "ускорен", "скорость", "давлен", "энерг", "мощност", "ток", "напряжен", "ньютон", "джоул"]):
            return "physics"
    ...
    def get_domain_instruction(self, domain: str) -> str:
        """Внутренняя стратегия подсказки без вывода предметной классификации пользователю."""
        instructions = {
            "geometry_proof": (
                "Начинай с чертежа и связей между объектами: что дают высоты, медианы, биссектрисы, "
                "параллельность, перпендикулярность, равные углы и стороны. Сформулируй, какой факт нужно доказать первым."
            ),
            "geometry_calculation": (
                "Сначала назови фигуру, данные и нужную величину. Предложи подходящую теорему или соотношение, "
                "не считай финальный численный ответ сразу."
            ),
            "planimetry": (
                "Начинай с чертежа, известных углов, сторон, высот, медиан, биссектрис и связей между точками. "
                "Не своди условие к абстрактному уравнению без геометрического смысла."
            ),
            "stereometry": (
                "Начинай с пространственной конфигурации: точки, плоскости, сечение, параллельность, "
                "перпендикулярность и расстояния. Дай первый геометрический шаг."
            ),
            "vectors": (
                "Начни с координат векторов или правил сложения: конец минус начало, затем линейная комбинация, "
                "затем длина или нужное свойство."
            ),
            "algebra_equation": (
                "Определи вид уравнения, проверь ограничения, если они есть, и предложи первый метод: "
                "разложение, замена, возведение в степень, ОДЗ или стандартная формула."
            ),
            "system": (
                "Начни с выбора переменной для исключения или подстановки, либо с геометрического/графического смысла. "
                "Не решай систему до конца сразу."
            ),
            "inequality": (
                "Начни с ОДЗ, переноса в одну сторону, критических точек, замены или метода интервалов. "
                "Финальный промежуток сразу не выдавай."
            ),
            "logarithms": (
                "Первый шаг - ОДЗ: аргументы логарифмов положительны, основания положительны и не равны 1. "
                "Затем выбирается свойство логарифмов или замена."
            ),
            "trigonometry": (
                "Начни с подходящего тождества или формулы сложения, затем отдельно учитывай отрезок/периодичность корней."
            ),
            "parameters": (
                "Начни с того, что означает требуемое число решений: единственное решение, касание, дискриминант, "
                "графическое пересечение или разбор случаев."
            ),
            "finance_credit": (
                "Начинай со схемы изменения величины по периодам: начисление процентов, платёж, остаток. "
                "Обозначь процент и запиши первое соотношение."
            ),
            "word_problem_motion": (
                "Обозначь скорости, используй расстояние = скорость * время, и составь первое уравнение по общей дистанции или разности расстояний."
            ),
            "word_problem_work": (
                "Введи производительность за единицу времени, затем используй работа = производительность * время."
            ),
            "word_problem_mixture": (
                "Отслеживай количество вещества: масса * концентрация или доля компонента."
            ),
            "word_problem_percent": (
                "Переводи r% в коэффициент 1 + r/100 или 1 - r/100 и записывай изменение по шагам."
            ),
            "probability": (
                "Определи события, что дано, что нужно найти, и выбери формулу: дополнение, произведение, сумма или условная вероятность."
            ),
            "combinatorics": (
                "Сначала реши, важен ли порядок и есть ли повторения, затем выбирай размещения, сочетания или перестановки."
            ),
            "functions_graphs": (
                "Начни с области определения, ключевых точек, монотонности или преобразований графика."
            ),
            "derivatives": (
                "Определи правило дифференцирования: сумма, произведение, частное, сложная функция или геометрический смысл касательной."
            ),
            "integrals": (
                "Определи тип: табличный интеграл, замена, по частям или геометрический смысл площади."
            ),
            "limits": (
                "Начни с подстановки, затем выбери метод: разложение, сокращение, замечательный предел или эквивалентность."
            ),
            "physics": (
                "Выпиши дано, переведи единицы, выбери формулу и покажи первый расчётный шаг."
            ),
            "chemistry": (
                "Выпиши дано, выбери формулу, работай через количество вещества, массу, объём или концентрацию."
            ),
            "informatics_algorithm": (
                "Объясни идею алгоритма, инвариант, шаги и сложность, не сводя всё к математическому уравнению."
            ),
            "programming": (
                "Помоги найти ошибку или идею решения, объясни причину и предложи минимальный следующий шаг."
            ),
            "general_math_theory": (
                "Дай краткий метод и типовую схему, не требуй конкретное условие."
            ),
        }
        return instructions.get(
            domain,
            "Определи данные, требуемый результат и предложи первый предметный шаг без финального ответа.",
        )
...
Только одно слово:"""
    ...
    def build_guided_reply(
        ...
        mode_instruction = (
            ...
            "Дай осмысленный первый шаг по условию и один наводящий вопрос."
...
Внутренняя стратегия подсказки. Не показывай пользователю название внутреннего типа задачи:
{domain_instruction}

{mode_instruction}

Требования к ответу:
1. Не выводи предметную классификацию и не начинай с фраз «это задача по...», «это уравнение или система», «это физика/химия/информатика».
2. Сначала коротко выдели данные, объекты или величины именно из этой задачи.
3. Если в условии есть треугольники, углы, высоты, медианы, биссектрисы, окружности, площади или доказательство, начни с чертежа и связей между объектами.
...
8. Формулы пиши простым текстом без LaTeX и markdown."""
    ...
    def build_generic_guidance(
        ...
        guidance_by_domain = {
            "geometry_proof": (
                "Условие вижу. Начнём с чертежа и связей между объектами. "
    ...
    def build_offline_reply(self, user_message: str, intent: str, attempt: int) -> str:
        ...
        if intent == "CHEATING":
            ...
            if attempt == 1:
                return "Дам метод, а не готовый ответ. Выпиши данные и запиши первое соотношение или первый вывод из условия."
            if attempt == 2:
